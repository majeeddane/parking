...
export default function SubscriptionCardPage() {
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: `بطاقة اشتراك مواقف - رقم الاشتراك ${userApplication?.subscriptionNumber || 'PARK-2026-10482'}`,
  ...
  const subNumber = userApplication?.subscriptionNumber || userApplication?.id || 'PARK-2026-10482';
  const memberName = currentUser?.fullName || 'محمد أحمد العتيبي';
  const vehicleName = userApplication ? `${userApplication.vehicleMake} ${userApplication.vehicleModel} ${userApplication.vehicleYear}` : 'Toyota Camry 2024';
  const plateNumber = userApplication?.plateNumber || 'أ ب ج 1234';
  const startDate = userApplication?.subscriptionStartDate || '01 سبتمبر 2026';
  const endDate = userApplication?.subscriptionEndDate || '31 أغسطس 2027';
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-center relative z-10">
                  ...
                  <div className="sm:col-span-2 space-y-4">
                    ...
                    <div className="pt-2 border-t border-white/10 flex items-center justify-between text-xs text-white/80">
                      ...
                      <div className="text-left">
                        ...
                        <span className="font-medium text-emerald-200">{endDate}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center p-3.5 bg-white rounded-2xl shadow-lg border border-white/40 text-center">
                    ...
                    <span className="text-[9px] text-slate-400 font-mono" dir="ltr">{subNumber}</span>
                  </div>
              ...
              <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
                ...
                <div className="space-y-3 text-xs md:text-sm text-slate-600">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <p>ستتعرف كاميرات القراءة التلقائية على رقم اللوحة المسجل (<strong className="text-slate-800">{plateNumber}</strong>).</p>
                  </div>
                  <div className="flex items-start gap-3">
